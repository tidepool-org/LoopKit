...
public protocol RemoteDataService: Service {
    ...
    /// The maximum number of pump data to upload to the remote data service at one time.
    var pumpDataLimit: Int? { get }

    /**
     Upload pump data to the remote data service.

     - Parameter stored: The stored pump data to upload.
     - Parameter completion: The completion function to call with any success or failure.
     */
    func uploadPumpData(_ stored: [SyncPumpEvent], completion: @escaping (_ result: Result<Bool, Error>) -> Void)
...
public extension RemoteDataService {
    ...
    var pumpDataLimit: Int? { return nil }
